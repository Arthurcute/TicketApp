package com.example.ticket.network;
...
import com.example.ticket.activity.RegisterActivity;
...
import com.example.ticket.model.Section;
...
public class DataManager {
                        ...
                        try {
                            ...
                            for (int i = 0; i < response.length(); i++) {
                                ...
                                TicketType ticketType = new TicketType(
                                        ...
                                        jsonObject.getString("type_name"), // Thay từ "section_name" thành "type_name"
                                        jsonObject.getDouble("price"),
                                        jsonObject.getInt("total_quantity"), // Thay từ "quantity" thành "total_quantity"
                                        jsonObject.getInt("available_quantity") // Thêm "available_quantity"
                                );
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        try {
                            JSONObject jsonObject = new JSONObject(response);
                            int conversationId = jsonObject.optInt("conversation_id", -1); // Lấy conversation_id nếu có
                            boolean exists = conversationId != -1; // Kiểm tra cuộc trò chuyện có tồn tại không

                            // Tạo một đối tượng Conversation
                            Conversation conversation = new Conversation(conversationId, eventId, userId, exists);
    ...
    public void createConversation(int eventId, String userId, String userName, ConversationCallback callback) {
        Log.d("DataManager createConversation", "EventId: " + eventId + ", UserId: " + userId + ", UserName: " + userName);
        String url = Utils.ROOT_URL + "/createConversation.php"; // Đường dẫn đến script PHP tạo cuộc trò chuyện
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, url, jsonObject,
                ...
                error -> {
                    String errorMessage;
                    if (error.networkResponse != null && error.networkResponse.data != null) {
                        // Lấy thông tin phản hồi từ server nếu có
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        try {
                            for (int i = 0; i < response.length(); i++) {
                                ...
                                Message message = new Message(messageId, convId, senderType, messageContent, timestamp);
