...
import com.example.ticketnew.adapter.TicketTypeAdapter;
...
public class DetailEventActivity extends AppCompatActivity {
    ...
    private TextView buyTicket, nameEvent, timeEvent, addressEvent, desEvent, timeTicket, soLuongVe;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        comment.setOnClickListener(view -> {
            ...
            String userIdString = preferences.getString("userId", null);

            if (userIdString != null && !userIdString.isEmpty()) {
                try {
                    int userId = Integer.parseInt(userIdString);
                    DataManager.getInstance(this).checkUserBlockStatus(userId, new DataManager.BlockStatusCallback() {
                        @Override
                        public void onStatusReceived(String message) {
                            // Hiển thị thông báo dựa trên trạng thái
                            Log.d("Message trạng thái user: ",message);
                            if (message.contains("bị chặn")) {
                                // Hiển thị thông báo người dùng bị chặn
                                AlertDialog.Builder builder = new AlertDialog.Builder(DetailEventActivity.this);
                                builder.setMessage("Bạn đã bị chặn nhắn tin!")
                                        .setCancelable(false) // Không thể nhấn ngoài để tắt
                                        .setPositiveButton("Đóng", (dialog, id) -> dialog.dismiss()); // Nút Đóng

                                AlertDialog alert = builder.create();
                                alert.show();
                            } else {
                                // Người dùng không bị chặn
                        ...
                        public void onError(String error) {
                            ...
                            Log.e("Lỗi kiểm tra người dùng có bị chặn không: ",""+ error);
                        }
                    });
                } catch (NumberFormatException e) {
                    Log.e("Lỗi:","User Id không hợp lệ");
                }
            } else {
                Log.e("Lỗi:","User Id không tồn tại");
            }
        });
            ...
            DataManager.getInstance(this).fetchLocationsByEventId(locationId, new DataManager.LocationCallback() {
                ...
                public void onSuccess(List<Location> locationList) {
                    if (!locationList.isEmpty()) {
                        ...
                        String sucChua = "Tổng vé: "+location.getCapacity();

                        addressEvent.setText(locationDetails);
                        soLuongVe.setText(sucChua);
